 ...
 * 分区状态机抽象类 定义了诸如 startup | shutdown 这样的公共方法 同时也给出了处理分区状态转换入口方法 handleStateChanges 的签名
 * PartitionStateMachine 和 ReplicaStateMachine的定义几乎相同
 *
...
abstract class PartitionStateMachine(controllerContext: ControllerContext) extends Logging {
   ...
   * @param partitions
   * @param targetState
   * @param leaderElectionStrategy
   * @return
   */
  def handleStateChanges(partitions: Seq[TopicPartition],
                         targetState: PartitionState,
                         leaderElectionStrategy: Option[PartitionLeaderElectionStrategy]): Map[TopicPartition, Either[Throwable, LeaderAndIsr]]
 ...
 * 即每个 Broker 进程启动时会在创建 KafkaController 对象的过程中 生成 ZkPartitionStateMachine 实例 而只有 Controller 组件所在的 Broker 才会启动分区状态机
 *
 * @param config
...
class ZkPartitionStateMachine(config: KafkaConfig,
  ...
  private def doHandleStateChanges(partitions: Seq[TopicPartition],
    ...
    // 代码根据不同 PartitionState 中定义的合法前置状态集合以及传入的目标状态(targetState) 将给定的副本对象集合划分成两部分
    // 1.能够合法转换的副本对象集合
    // 2.执行非法状态转换的副本对象集合
    ...
    targetState match {
      ...
      case OnlinePartition =>
        ...
        if (uninitializedPartitions.nonEmpty) {
          // 在ZK中创建并写入分区节点数据 节点的位置是/brokers/topics/topic名称/partitions/partition序号 每个节点都要包含分区的 Leader 和 ISR 等数据
        ...
        if (partitionsToElectLeader.nonEmpty) {
          ...
          electionResults.foreach {
            case (partition, Right(leaderAndIsr)) =>
              ...
              controllerContext.putPartitionState(partition, OnlinePartition)
            case (_, Left(_)) => // Ignore; no need to update partition state on election error. 如果选举失败忽略
          }
          // 返回Leader选举结果
          electionResults
        } else {
  ...
  private def initializeLeaderAndIsrForPartitions(partitions: Seq[TopicPartition]): Seq[TopicPartition] = {
    ...
    // 按照有无存活副本将分区分为两组: 有存活副本的分区 无任何存活副本的分区
   ...
   * 该方法会不断尝试为多个分区选举 Leader 直到所有分区都成功选出 Leader
   *
  ...
  private def doElectLeaderForPartitions(
      ...
      if (getDataResponse.resultCode == Code.OK) {
        TopicPartitionStateZNode.decode(getDataResponse.data, getDataResponse.stat) match {
          case Some(leaderIsrAndControllerEpoch) =>
            // 如果节点数据的Controller Epoch值大于当前Controller Epoch值 说明此节点已经被一个更新的Controller选举为Leader了
    ...
    // 是否包含可选举 Leader 的分区 如果一个满足选举 Leader 的分区都没有方法直接返回
    if (validLeaderAndIsrs.isEmpty) {
      return (failedElections.toMap, Seq.empty)
    }

    // 第2部分
    // 开始选举Leader 根据有无Leader将分区进行划分
    ...
    // 第3部分
    // 主要是更新 ZooKeeper 节点数据以及 Controller 端元数据缓存信息(ControllerContext)
    ...
    finishedUpdates.forKeyValue { (partition, result) =>
      result.foreach { leaderAndIsr =>
        ...
        val leaderIsrAndControllerEpoch = LeaderIsrAndControllerEpoch(leaderAndIsr, controllerContext.epoch)
        controllerContext.putPartitionLeadershipInfo(partition, leaderIsrAndControllerEpoch)
        // handleStateChanges 方法的 Controller 给相关的 Broker 发送请求的请求封装过程 调用位于doElectLeaderForPartitions中
        // 具体要给哪些 Broker 发送哪些请求 就是在该方法中完成的 主要目的有2个:
        // 1.创建发送leaderIsrRequest所必须的数据结构 将其添加到Controller待发送请求集合中
        // 2.创建发送updateMetadataRequest所必须的数据结构 将其添加到Controller待发送请求集合中
        controllerBrokerRequestBatch.addLeaderAndIsrRequestForBrokers(recipientsPerPartition(partition), partition,
          leaderIsrAndControllerEpoch, replicaAssignment, isNew = false)
      }
    }
    // 返回选举结果 包括成功选举并更新ZooKeeper节点的分区列表 选举失败分区列表以及ZooKeeper节点更新失败的分区列表
...
/**
 * 分区 Leader 选举的算法实现 为PartitionLeaderElectionStrategy提供了 4 类选举策略的实现代码
 * Kafka 为分区选举 Leader 的大体思路基本上就是: 找出 AR 列表(或给定副本列表)中首个处于存活状态且在 ISR 列表的副本 将其作为新 Leader
 */
object PartitionLeaderElectionAlgorithms {

  /**
   * 由于 Leader 副本下线而引发的分区 Leader 选举
   *
   * @param assignment                   分区的副本列表 该列表有个专属的名称(Assigned Replicas 简称 AR)
   *                                     当创建Topic之后 使用 kafka-topics 脚本查看Topic时 可以看到名为 Replicas 的一列数据
   *                                     这列数据显示的就是主题下每个分区的 AR assignments 参数类型是 Seq[Int] 说明AR 是有顺序的而且不一定和 ISR 的顺序相同
   * @param isr                          保存了分区所有与 Leader 副本保持同步的副本列表(Leader 副本自己也在 ISR 中) 作为 Seq[Int]类型的变量 isr 自身也是有顺序的
   * @param liveReplicas                 该分区下所有处于存活状态的副本 可以根据 Controller 元数据缓存中的数据来判定副本是否存活(所有在运行中的 Broker 上的副本都被认为是存活的)
   * @param uncleanLeaderElectionEnabled 在默认配置下只要不是由 AdminClient 发起的 Leader 选举 该值一般是 false(即 Kafka 不允许执行 Unclean Leader 选举)
   *                                     Unclean Leader 选举是指在 ISR 列表为空的情况下 Kafka 选择一个非 ISR 副本作为新的 Leader
   *                                     由于存在丢失数据的风险 社区目前已经通过把 Broker 端参数 unclean.leader.election.enable 的默认值设置为 false 的方式 禁止 Unclean Leader 选举了
   *                                     社区于 2.4.0.0 版本正式支持在 AdminClient 端为给定分区选举 Leader
   *                                     如果 Leader 选举是由 AdminClient 端触发的 那就默认开启 Unclean Leader 选举
   * @param controllerContext
   * @return
   */
  ...
  /**
   * 由于执行 Preferred 副本 Leader 选举而引发的分区 Leader 选举
   *
   * @param assignment
   * @param isr
   * @param liveReplicas
...
/**
 * 为 Kafka Topic的某个分区推选 Leader 副本
 * 定义 4 类分区 Leader 选举策略即 Leader 选举的 4 种场景
 */
...
/**
 * 定义 4 类分区状态 分别是 NewPartition | OnlinePartition | OfflinePartition | NonExistentPartition
 * 除此之外还定义了它们之间的流转关系
 */
sealed trait PartitionState {
  ...
  // 合法前置状态集合 从合法状态集以外的状态向目标状态进行转换将被视为非法操作
  def validPreviousStates: Set[PartitionState]
}

/**
 * 分区被创建后被设置成这个状态 表明它是一个全新的分区对象
 * 处于这个状态的分区被 Kafka 认为是"未初始化" 因此不能选举 Leader
 */
