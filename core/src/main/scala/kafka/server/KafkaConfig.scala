...
object KafkaConfig {
  ...
  // 控制是否设置Control Plane监听器的关键 一旦设置了该值 controlPlaneListener方法会记录下这个值 然后传入到SocketServer的createControlPlaneAcceptorAndProcessor方法中
...
/**
 * 封装了Broker端所有参数的信息 同时还定义了很多实用的工具方法
 *
 * @param props
 * @param doLog
...
class KafkaConfig(val props: java.util.Map[_, _], doLog: Boolean, dynamicConfigOverride: Option[DynamicBrokerConfig])
  ...
  /**
   * 获取 Control Plane EndPoint监听器 第2步
   * 返回<CONTROLLER, PLAINTEXT>
   *
   * @return
   */
  ...
  /**
   * 获取 Control Plane EndPoint 监听器第三步
   *
   * @return
   */
  def controlPlaneListener: Option[EndPoint] = {
    controlPlaneListenerName.map { listenerName =>
      // 读取listeners参数值 找出CONTROLLER对饮的监听器
  ...
  /**
   * 获取 Control Plane EndPoint 监听器的第1步
   *
   * @return
   */
  private def getControlPlaneListenerNameAndSecurityProtocol: Option[(ListenerName, SecurityProtocol)] = {
    // 获取 control.plane.listener.name 配置的值 并读取该值 用于判断Kafka是否启用了control plane监听器
    Option(getString(KafkaConfig.ControlPlaneListenerNameProp)) match {
      case Some(name) =>
        // 如果设置了该参数说明启用了 Control Plane监听器
        val listenerName = ListenerName.normalised(name)
        // 必须同时设置Broker端参数listener.security.protocol.map 并从该参数值中提取出该监听器对应的安全认证协议
        val securityProtocol = listenerSecurityProtocolMap.getOrElse(listenerName,
